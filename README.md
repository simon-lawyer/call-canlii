...
[CanLii](www.canlii.org) is an indispensible research tool for lawyers,
...
that allows (limited) programattic access to its databases. Too few
...
``` python
##### Check to see which legislation the Court cites
```
...
This program is designed to support quieries at scale. If you wanted to
get all of the keywords for the last ten SCC decisions, that can be
simply done.
...
Now we loop over the list, changing the case ID each time and printing
out the keywords.
